...
description: What ayx one covers — workflows, connections, datasets, jobs, scheduling, and platform administration.
sidebar:
  order: 0
---
...
Because these are separate technologies rather than separate views of the same data, moving work between them is a **migration**, not a configuration change. Commands that cross the boundary say so explicitly — see `ayx designer workflow migrate` and the cloud-conversion notes in [Workflows & packages](/server/workflow/).
...
# Actually deletes
ayx one workflows delete <id> --apply
```
...
Pass `-o json` for machine-readable output on stdout. The historical `--output json` spelling remains supported, and the global flag can appear before or after the subcommand:
...
```bash
ayx one workflows list --profile staging
```
