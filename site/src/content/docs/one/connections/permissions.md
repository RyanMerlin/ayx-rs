...
ayx -o json one connections permissions list <id>
```
...
ayx -o json one connections list --all | jq -r '.data.items[].id' > conn-ids.txt

# Then revoke per connection where the subject appears
while read conn_id; do
ayx -o json one connections permissions list "$conn_id" \
