...
use crate::{
    ServerApiCommand, ServerAuthCommand, ServerAuthDiagnoseCommand, ServerAuthSimulateCommand,
    ServerCommand, ServerDiagnoseCommand, ServerDoctorCommand, ServerLogsCommand, UpgradeCommand,
    ...
    Ok(match command {
        ...
        ServerCommand::ServerLogs { command } => match command {
            ...
            ServerLogsCommand::Inventory { profile } => {
                let config = load_profile!(profile.as_deref(), environment)?;
                Envelope::ok_with_data("log inventory discovered", discover_log_inventory(&config))
            }
                ...
                if let Some(path) = log_file {
                    ...
                    if let Some(error_text) = error.as_ref() {
                        evidence["error_context"] =
                            json!(extract_context(&path, error_text, 25, 25)?);
                    }
                    steps.push(json!({
                ...
                ServerAuthSimulateCommand::Saml {
                    ...
                    let parsed_metadata = metadata_url
                        ...
                        .or_else(|| {
                            metadata_file
                                .as_ref()
                                .map(|path| parse_saml_metadata_source(&path.display().to_string()))
                                .transpose()
                                .ok()
                                .flatten()
                        });
