...
use super::jobs::{fetch_job_groups, is_failure_status, pct, per_flow_stats, within_window};
...
pub fn errors(environment: Option<&str>, args: &TelemetryArgs) -> Result<Envelope> {
    ...
    let mut rows: Vec<Value> = stats
        ...
        .map(|(flow_id, name, _, run_count, failed)| {
            json!({
                "flow_id": flow_id,
                "flow_name": name,
                "run_count": run_count,
    ...
    rows.sort_by(|a, b| {
        b["failed"]
            .as_u64()
            .unwrap_or(0)
            .cmp(&a["failed"].as_u64().unwrap_or(0))
    });
    let take = args.top.max(1);
    let items: Vec<Value> = rows.into_iter().take(take).collect();
    ...
    Ok(Envelope::ok_with_data(
        format!(
            "telemetry workflows errors ({}): {} flow{} ({} failed jobs)",
            window.label,
            items.len(),
            if items.len() == 1 { "" } else { "s" },
            total_failed
