...
- **`ayx one api coverage` now reports wired endpoints that lie outside the spec's namespace.** The One gateway spec documents `/v4` only, and the CLI also speaks `/svc-workflow` plus the separate `/billing/v1` surface; the plans, scheduling, and workspace suspend/unsuspend commands were repointed to `/v4` paths in this branch. The rows that remain outside the spec namespace now appear under `outside_spec_namespace` alongside the commands that dispatch them, `inventory_total` counts every distinct wired row, and `coverage_pct` is documented as scoped to the comparable namespace rather than reading as "percent of the One surface covered". `--check` still gates on `missing` only: these endpoints are unverified by the diff, not drift.
...
- **Alteryx One API-coverage metadata drift**: `one api coverage` and `catalog describe` now report the endpoints the CLI actually calls. `one flows update` is recorded as `PATCH /v4/flows/{id}` (was `PUT`); `one platform workspace people`/`admins` map to `GET /v4/people` and `GET /v4/people?role=admin` (the `/v4/workspaces/{id}/people` and `/admins` routes 404 — workspace context is carried by the `x-alteryx-workspace-gid` header); the workspace suspend/unsuspend mappings are now `POST /v4/workspaces/{id}/people/suspend` and `POST /v4/workspaces/{id}/people/unsuspend`; and the `workspace people`/`admins` prerequisites are corrected from `server_api` to `alteryx_one.access_token`. The inventory dedup guard now pins each shared endpoint to its exact expected command set, so future accidental drift fails the test.
