...
- **`ayx one api coverage` now reports wired endpoints that lie outside the spec's namespace.** The One gateway spec documents `/v4` only, but the CLI also speaks `/svc-workflow`, `/plans/v1`, `/scheduling/v1`, `/billing/v1`, and `/iam/v1`. Those 25 rows (16% of the wired surface) were previously dropped from the report entirely — never covered, never missing, never stale — while `inventory_operations` under-reported a 153-row inventory as 128. They now appear under `outside_spec_namespace` alongside the commands that dispatch them, `inventory_total` counts every row, and `coverage_pct` is documented as scoped to the comparable namespace rather than reading as "percent of the One surface covered". `--check` still gates on `missing` only: these endpoints are unverified by the diff, not drift.
