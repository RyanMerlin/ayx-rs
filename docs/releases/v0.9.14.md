...
- `flows validate` (`/v4/flows/{id}/validate`) and `webhook-flow-tasks` (`/v4/webhookFlowTasks`) return 404 — these routes are not present on the `platform_packaging` tier.
- Billing, plans, and scheduling remain enterprise-tier-gated (404 on `platform_packaging`).
