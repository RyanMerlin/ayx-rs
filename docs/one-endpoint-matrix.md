...
| GET | `/v4/people/count` | live 410 IAM_ENDPOINT_SCREAM_TEST | 2026-08-17 | `one person count` | object: raw API count body (`{ count }`/`{ total }`, service-specific) | json:GoneException | Vendor scream-test removal: live 410 body reported `GoneException` with `IAM_SCREAM_PEOPLE`. There is no replacement count endpoint; use `one person list` for enumeration. |
