...
- Cleanup: connection, group, token, and workflow ID sets matched their baselines after reverse-order cleanup; no canary residue remained. Legacy recipe execution/profiling/write-settings, output-object CRUD, and `one job-groups run` were intentionally excluded. The CLI still has no execution or cancellation command for cloud-native workflow ULIDs; that remains a product/API gap.
