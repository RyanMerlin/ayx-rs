...
| Method | Path | Live status | Verified (UTC) | ayx command(s) | Response shape | Error-body flavor | Notes |
|---|---|---|---|---|---|---|---|
| GET | `/plans/v1/plans` | live 404 RouteNotFoundException | 2026-08-14T16:08Z | `one plans list` | paginated list: `{ items[], next_page_token, pages_fetched, page_envelopes[] }` (CLI-normalized) | json:RouteNotFoundException (same gateway error shape as `/v4`, confirmed live for plans/scheduling/billing) | Phase 2 confirmed a real 404 with 0 items; `one plans list` still reports top-level `"ok": true` because `page_envelopes[0].status_code` is the authoritative status. `one doctor plans` exposed the nested 404/`not_found`. This remains consistent with the tenant's likely missing Plans entitlement. |
...
| GET | `/plans/v1/plans/count` | live 404 RouteNotFoundException | 2026-08-14T16:08Z | `one plans count` | object: raw API count body (`{ count }`/`{ total }`, service-specific) | json:RouteNotFoundException (same gateway error shape as `/v4`, confirmed live for plans/scheduling/billing) | Phase 2 and `one doctor plans` confirmed 404 `not_found`/`RouteNotFoundException`. The live-smoke allowlist still covers `permission_denied` but not this tenant's `not_found`; this is the same tier-gated surface outcome as `/plans/v1/plans`, not a wiring failure. |
...
| Method | Path | Live status | Verified (UTC) | ayx command(s) | Response shape | Error-body flavor | Notes |
|---|---|---|---|---|---|---|---|
| GET | `/scheduling/v1/schedules` | live 404 RouteNotFoundException | 2026-08-14T16:13Z | `one scheduling list` | paginated list: `{ items[], next_page_token, pages_fetched, page_envelopes[] }` (CLI-normalized) | json:RouteNotFoundException (same gateway error shape as `/v4`, confirmed live for plans/scheduling/billing) | Phase 2 and `one doctor scheduling` confirmed the tier-gated 404. The list leaf still reports `"ok": true`, 0 items, while `page_envelopes[0].status_code` is 404. |
