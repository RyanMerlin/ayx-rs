...
| GET | `/v4/connections/{id}` | live 200 | 2026-08-14T16:09Z | `one connections detail` | object: raw API resource body, JSON-passthrough | json:ApiValidationFailed / json:RouteNotFoundException / json:AccessControlException (Alteryx One `/v4` gateway shape) | Phase 2 detail of a disposable connection returned 200. |
| GET | `/v4/connections/{id}/status` | live 200 | 2026-07-27T00:55Z | `one connections status` | object: raw API resource body, JSON-passthrough | json:ApiValidationFailed / json:RouteNotFoundException / json:AccessControlException (Alteryx One `/v4` gateway shape) | Disposable connection status returned 200. |
