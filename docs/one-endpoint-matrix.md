...
`200`. Those raw transport fields are available with `-o json`: check
...
| GET | `/v4/people` | live 200 | 2026-08-18T17:55Z | `one person list`<br>`one workspace members list` | paginated list: `{ items[], next_page_token, pages_fetched, page_envelopes[] }` (CLI-normalized) | json:ApiValidationFailed / json:RouteNotFoundException / json:AccessControlException (Alteryx One `/v4` gateway shape) | Both commands, and telemetry's `permissions workflows/summary --source one`, use this header-scoped route with `x-alteryx-workspace-gid`; the old `/v4/workspaces/{id}/people` route returns 404. |
| GET | `/v4/workspaces/{workspaceId}/admins` | declared in the tenant OpenAPI spec; live re-verification pending | 2026-08-31 | `one workspace members admins` | object: raw API resource body, JSON-passthrough | json:ApiValidationFailed / json:RouteNotFoundException / json:AccessControlException (Alteryx One `/v4` gateway shape) | `workspaceId` is the numeric workspace id, not the workspace GID. |
| GET | `/v4/workspaces/{id}/groups` | live 200 | 2026-08-18T18:08Z | `one workspace groups list` | object: raw API resource body, JSON-passthrough | json:ApiValidationFailed / json:RouteNotFoundException / json:AccessControlException (Alteryx One `/v4` gateway shape) | Numeric disposable workspace returned 200. |
| GET | `/v4/workspaces/{id}/invitationLink` | live 200 | 2026-08-18T18:08Z | `one workspace members invitation-link` | object: raw API resource body, JSON-passthrough | json:ApiValidationFailed / json:RouteNotFoundException / json:AccessControlException (Alteryx One `/v4` gateway shape) | The command supplies required query `personId`; synthetic workspace and person fixtures returned 200. |
| GET | `/v4/workspaces/{workspaceId}/cloudConfigs` | live 200 | 2026-08-18T18:08Z | `one workspace cloud-configs list` | object: raw API resource body, JSON-passthrough | json:ApiValidationFailed / json:RouteNotFoundException / json:AccessControlException (Alteryx One `/v4` gateway shape) | A numeric disposable workspace returned 200. |
| DELETE | `/v4/workspaces/{workspaceId}/people/{id}` | unverified | not probed this session | `one workspace members remove` | object: mutation result / dry-run shape (`{ dry_run, mutating, would_send }` when not `--apply`) | json:ApiValidationFailed / json:RouteNotFoundException / json:AccessControlException (Alteryx One `/v4` gateway shape) | Mutating — not probed. |
| POST | `/v4/workspaces/{id}/people/unsuspend` | unverified | not probed since repoint | `one workspace members unsuspend` | object: mutation result / dry-run shape (`{ dry_run, mutating, would_send }` when not `--apply`) | json:ApiValidationFailed / json:RouteNotFoundException / json:AccessControlException (Alteryx One `/v4` gateway shape) | Vendor operation is workspace-wide; not yet re-probed live. |
| PATCH | `/v4/workspaces/{id}/transfer` | unverified | not probed this session | `one workspace transfer start` | object: mutation result / dry-run shape (`{ dry_run, mutating, would_send }` when not `--apply`) | json:ApiValidationFailed / json:RouteNotFoundException / json:AccessControlException (Alteryx One `/v4` gateway shape) | Corrected to the live spec method. Mutating — not probed. |
...
> `one workflows runs` is intentionally not exposed. Cloud-native workflow execution returns both `jobId` and `jobgroupId`; `jobId` is for Workflow Service cancellation, while `jobgroupId` is passed to the canonical `one jobs runs` command, which reads the provider's Job Group child-run collection. The CLI does not infer history by scanning all jobs.
...
| GET | `/svc-workflow/api/v1/assets` | live 200 | 2026-09-15 | `one workflows assets`<br>`one workflows detail`<br>`one workflows graph` | paginated list: `{ items[], next_page_token, pages_fetched, page_envelopes[] }` (CLI-normalized) | json:NotFoundError (`{ kind: "NotFoundError", message, errors: [], params: {} }`) for a valid route / unknown resource id | Live canary workflow detail and graph reads succeeded. The provider supplied no graph data, so normalized graph buckets were empty and `available: false`; no field schemas were inferred. |
...
| POST | `/svc-workflow/api/v1/workflows/{id}/run` | live 200 | 2026-09-15 | `one workflows run` | object: provider run/job result (`jobId`, `jobStatus`, `jobgroupId`) / dry-run shape (`{ dry_run, mutating, would_send }` when not `--apply`) | json:BadRequestError (`{ kind: "BadRequestError", message, errors: [], params: {} }`) for an invalid workflow | Live disposable copy was queued after a successful dry-run; the provider returned both `jobId` and `jobgroupId` (`jobStatus: ACCEPTED`). The copy was deleted after the canary. |
| POST | `/svc-workflow/api/v1/jobs/{id}/cancel` | live 404 capability-blocked | 2026-09-15 | `one workflows cancel` | object: provider cancellation result / dry-run shape (`{ dry_run, mutating, would_send }` when not `--apply`) | json:NotFoundError (`{ kind: "NotFoundError", message, errors: [], params: {} }`) | `id` is the `jobId` returned by `one workflows run`, not the workflow definition ULID or the Job Group id. The live canary reached the route with that id, but this workspace returned `WFS Jobs is not enabled in this environment`; cancellation remains blocked by provider capability here. |
| POST | `/svc-workflow/api/v2/workflows/{id}/duplicate` | live 201 | 2026-09-15 | `one workflows copy` | object: mutation result / dry-run shape (`{ dry_run, mutating, would_send }` when not `--apply`) | json:NotFoundError (`{ kind: "NotFoundError", message, errors: [], params: {} }`) for a valid route / unknown resource id | Live disposable workflow copy succeeded after dry-run, was inspected and run, then deleted by captured ID. |
...
| DELETE | `/svc-workflow/api/v2/workflows/{id}` | live 200 | 2026-09-15 | `one workflows delete` | empty object `{}` on success (HTTP 200) / dry-run shape (`{ dry_run, mutating, would_send: null }` when not `--apply`) | json:NotFoundError (`{ kind: "NotFoundError", message, errors: [], params: {} }`) for a valid route / unknown resource id | Deleted the live canary copy by ID; a follow-up detail read returned not-found and the original workflow baseline remained present. |
...
| Method | Path | Live status | Verified (UTC) | ayx command(s) | Response shape | Error-body flavor | Notes |
|---|---|---|---|---|---|---|---|
| GET | `/v4/jobLibrary` | live 200 | 2026-08-14T16:11Z | `one jobs list` | paginated list: `{ items[], next_page_token, pages_fetched, page_envelopes[] }` (CLI-normalized) | json:ApiValidationFailed / json:RouteNotFoundException / json:AccessControlException (Alteryx One `/v4` gateway shape) | Phase 2 returned 25 items. |
| GET | `/v4/jobLibrary/count` | live 200 | 2026-07-27T00:55Z | `one jobs count` | object: raw API count body (`{ count }`/`{ total }`, service-specific) | json:ApiValidationFailed / json:RouteNotFoundException / json:AccessControlException (Alteryx One `/v4` gateway shape) | `one jobs count`. |
| POST | `/v4/jobGroups` | unverified | not probed this session | `one jobs execute` | object: mutation result / dry-run shape (`{ dry_run, mutating, would_send }` when not `--apply`) | json:ApiValidationFailed / json:RouteNotFoundException / json:AccessControlException (Alteryx One `/v4` gateway shape) | Mutating (`one jobs execute`) — not probed. |
| PUT | `/v4/jobGroups/{id}/publish` | unverified | not probed this session | `one jobs publish` | object: mutation result / dry-run shape (`{ dry_run, mutating, would_send }` when not `--apply`) | json:ApiValidationFailed / json:RouteNotFoundException / json:AccessControlException (Alteryx One `/v4` gateway shape) | Mutating — not probed. |
| GET | `/v4/jobGroups/{id}` | live 200 | 2026-08-14T16:11Z | `one jobs` | object: raw API resource body, JSON-passthrough | json:ApiValidationFailed / json:RouteNotFoundException / json:AccessControlException (Alteryx One `/v4` gateway shape) | Phase 2 detail of a disposable job group returned 200. |
| POST | `/v4/jobGroups/{id}/cancel` | unverified | not probed this session | `one jobs cancel` | object: mutation result / dry-run shape (`{ dry_run, mutating, would_send }` when not `--apply`) | json:ApiValidationFailed / json:RouteNotFoundException / json:AccessControlException (Alteryx One `/v4` gateway shape) | Mutating — not probed. |
| GET | `/v4/jobGroups/{id}/status` | live 200 | 2026-07-27T00:55Z | `one jobs status` | object: raw API resource body, JSON-passthrough | json:ApiValidationFailed / json:RouteNotFoundException / json:AccessControlException (Alteryx One `/v4` gateway shape) | Disposable job-group status returned 200. |
| GET | `/v4/jobGroups/{id}/inputs` | live 400 DataServiceInvalidRequest | 2026-07-27T00:55Z | `one jobs inputs` | object: raw API resource body, JSON-passthrough | json:ApiValidationFailed / json:RouteNotFoundException / json:AccessControlException (Alteryx One `/v4` gateway shape) | A disposable job group's inputs call returned 400, `"Illegal Argument: Only Jdbc sources have connect String"`. Route exists; this is a data-shape-specific validation error, not a CLI wiring bug. |
| GET | `/v4/jobGroups/{id}/pdfResults` | live 400 ProfilingDataNotFoundException | 2026-07-27T00:55Z | `one jobs pdf-results` | object: raw API resource body, JSON-passthrough | json:ApiValidationFailed / json:RouteNotFoundException / json:AccessControlException (Alteryx One `/v4` gateway shape) | The disposable job group returned 400 because it has no profiling data. Route exists; the fixture simply has no profiling artifact. |
| GET | `/v4/jobGroups/{id}/outputs` | live 200 | 2026-07-27T00:55Z | `one jobs outputs` | object: raw API resource body, JSON-passthrough | json:ApiValidationFailed / json:RouteNotFoundException / json:AccessControlException (Alteryx One `/v4` gateway shape) | Disposable job-group outputs returned 200. |
| GET | `/v4/jobGroups/{id}/jobs` | live 200 | 2026-09-15 | `one jobs runs` | object: raw API resource body, JSON-passthrough | json:ApiValidationFailed / json:RouteNotFoundException / json:AccessControlException (Alteryx One `/v4` gateway shape) | Live cloud-native workflow run's `jobgroupId` returned one child-run record. This is the provider-backed run-history path; no `one workflows runs` command is synthesized. |
| GET | `/v4/jobGroups/{id}/publications` | live 200 | 2026-07-27T00:55Z | `one jobs publications` | object: raw API resource body, JSON-passthrough | json:ApiValidationFailed / json:RouteNotFoundException / json:AccessControlException (Alteryx One `/v4` gateway shape) | Disposable job-group publications returned 200. |
| GET | `/v4/jobGroups/{id}/profile` | live 400 ProfilingDataNotFoundException | 2026-07-27T00:55Z | `one jobs profile` | object: raw API resource body, JSON-passthrough | json:ApiValidationFailed / json:RouteNotFoundException / json:AccessControlException (Alteryx One `/v4` gateway shape) | The disposable job group returned 400 for the same "no profiling data" reason as `pdfResults`. |
| GET | `/v4/jobGroups/{id}/profileResults` | live 400 ProfilingDataNotFoundException | 2026-07-27T00:55Z | `one jobs profile-results` | object: raw API resource body, JSON-passthrough | json:ApiValidationFailed / json:RouteNotFoundException / json:AccessControlException (Alteryx One `/v4` gateway shape) | The disposable job group returned 400 for the same "no profiling data" reason. |
...
| GET | `/v4/people` | live 200 | 2026-08-18T17:55Z | `one person list`<br>`one workspace members list` | paginated list: `{ items[], next_page_token, pages_fetched, page_envelopes[] }` (CLI-normalized) | json:ApiValidationFailed / json:RouteNotFoundException / json:AccessControlException (Alteryx One `/v4` gateway shape) | Both commands use the header-scoped route with `x-alteryx-workspace-gid`; the old `/v4/workspaces/{id}/people` route returns 404. |
...
| GET | `/v4/workspaces/current/configuration` | live 200 | 2026-07-27T00:55Z | `one workspace config get` | object: raw API resource body, JSON-passthrough | json:ApiValidationFailed / json:RouteNotFoundException / json:AccessControlException (Alteryx One `/v4` gateway shape) | Current-workspace configuration only. |
| PATCH | `/v4/workspaces/current/configuration` | unverified | not probed this session | `one workspace config set` | object: mutation result / dry-run shape (`{ dry_run, mutating, would_send }` when not `--apply`) | json:ApiValidationFailed / json:RouteNotFoundException / json:AccessControlException (Alteryx One `/v4` gateway shape) | Mutating — not probed. |
| GET | `/v4/workspaces/current/configuration-schema` | live 200 | 2026-07-27T00:55Z | `one workspace config schema` | object: raw API resource body, JSON-passthrough | json:ApiValidationFailed / json:RouteNotFoundException / json:AccessControlException (Alteryx One `/v4` gateway shape) | Current-workspace schema only. |
| POST | `/v4/workspaces/current/delete-configuration` | unverified | not probed this session | `one workspace config reset` | object: mutation result / dry-run shape (`{ dry_run, mutating, would_send }` when not `--apply`) | json:ApiValidationFailed / json:RouteNotFoundException / json:AccessControlException (Alteryx One `/v4` gateway shape) | Mutating and destructive — not probed. |
...
| POST | `/v4/workspaces/{id}/groups` | live 200 | 2026-08-18T20:23Z | `one workspace groups create` | object / dry-run envelope | json:ApiValidationFailed / json:RouteNotFoundException / json:AccessControlException | Created a disposable group; body requires `name` and `members`. |
| DELETE | `/v4/workspaces/{id}/groups/{groupId}` | live 201 | 2026-08-18T20:23Z | `one workspace groups delete` | object / dry-run envelope | json:ApiValidationFailed / json:RouteNotFoundException / json:AccessControlException | Deleted the disposable group after removing both users; workspace returned to the original one-group baseline. |
| PUT | `/v4/workspaces/{id}/groups/{groupId}` | unverified | not probed this session | `one workspace groups update` | object: mutation result / dry-run shape (`{ dry_run, mutating, would_send }` when not `--apply`) | json:ApiValidationFailed / json:RouteNotFoundException | Mutating — JSON body requires `name`. |
| PUT | `/v4/workspaces/{id}/groups/{groupId}/roles` | live 2xx | 2026-09-01T21:44Z | `one workspace groups roles set` | object: mutation result / dry-run shape | json:ApiValidationFailed | End-to-end canary assigned Viewer policy `25704008` to a temporary group and removed it again. |
| POST | `/v4/workspaces/{id}/groups/{groupId}/users` | live 201 | 2026-08-18T20:23Z | `one workspace groups members add` | object / dry-run envelope | json:ApiValidationFailed | Added two disposable users using repeated `userIds` query parameters. |
| DELETE | `/v4/workspaces/{id}/groups/{groupId}/users` | live 201 | 2026-08-18T20:23Z | `one workspace groups members remove` | object / dry-run envelope | json:ApiValidationFailed | Removed both disposable memberships before group deletion. |
| POST | `/v4/workspaces/{id}/people` | unverified | not probed this session | `one workspace members invite` | object: mutation result / dry-run envelope | json:ApiValidationFailed | Mutating — JSON body requires `email`; target person must be selected before live testing. |
| PATCH | `/v4/workspaces/{id}/people/batch` | unverified | not probed this session | `one workspace members reinvite` | object: mutation result / dry-run envelope | json:ApiValidationFailed | Mutating — JSON body requires `personIds`. |
| PUT | `/v4/workspaces/{id}/people/{personId}/suspended` | unverified | not probed this session | `one workspace members suspend` | object: mutation result / dry-run envelope | json:ApiValidationFailed | Mutating — target person must be selected before live testing. |
| POST | `/v4/workspaces/{workspaceId}/cloudConfigs/{cloudProvider}` | unverified | not probed this session | `one workspace cloud-configs create` | object: mutation result / dry-run envelope | json:ApiValidationFailed | Mutating — JSON body is provider-specific. |
| PATCH | `/v4/workspaces/{workspaceId}/cloudConfigs/{cloudProvider}` | unverified | not probed this session | `one workspace cloud-configs update` | object: mutation result / dry-run envelope | json:ApiValidationFailed | Mutating — JSON body is provider-specific. |
| PATCH | `/v4/workspaces/{workspaceId}/people/{id}` | unverified | not probed this session | `one workspace members update` | object: mutation result / dry-run envelope | json:ApiValidationFailed | Mutating — target person must be selected before live testing. |
...
- Cleanup: connection, group, token, and workflow ID sets matched their baselines after reverse-order cleanup; no canary residue remained. Legacy recipe execution/profiling/write-settings, output-object CRUD, and `one jobs execute` remain intentionally excluded. No cloud-native ULID was sent to a legacy endpoint.
