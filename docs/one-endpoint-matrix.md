...
| GET | `/v4/workspaces/{id}/groups` | live 200 | 2026-08-18T18:08Z | `one workspace groups` | object: raw API resource body, JSON-passthrough | json:ApiValidationFailed / json:RouteNotFoundException / json:AccessControlException (Alteryx One `/v4` gateway shape) | Numeric workspace `91946` returned 200. |
...
| GET | `/v4/workspaces/{id}/invitationLink` | live 200 | 2026-08-18T18:08Z | `one workspace invitation-link` | object: raw API resource body, JSON-passthrough | json:ApiValidationFailed / json:RouteNotFoundException / json:AccessControlException (Alteryx One `/v4` gateway shape) | The command supplies required query `personId`; numeric workspace `91946` and person `249085` returned 200. |
| GET | `/v4/workspaces/{workspaceId}/cloudConfigs` | live 200 | 2026-08-18T18:08Z | `one workspace cloud-configs` | object: raw API resource body, JSON-passthrough | json:ApiValidationFailed / json:RouteNotFoundException / json:AccessControlException (Alteryx One `/v4` gateway shape) | Numeric workspace `91946` returned 200. |
...
| GET | `/v4/authorization/roles` | live 200 | 2026-08-18T18:08Z | `one role list` | object: raw API resource body, JSON-passthrough | json:ApiValidationFailed / json:RouteNotFoundException / json:AccessControlException (Alteryx One `/v4` gateway shape) | Current live probe returned 200. |
...
| GET | `/v4/authorization/roles/{id}/people` | live 403 permission_denied | 2026-08-18T17:55Z | `one role list-assignments` | paginated list: `{ items[], next_page_token, pages_fetched, page_envelopes[] }` (CLI-normalized) | json:AccessControlException | Fake role id `0` reached the authenticated route and returned 403 `permission_denied`; route existence is confirmed, but this profile lacks authorization. |
| PUT | `/v4/authorization/roles/{id}/people` | unverified | not probed this session | `one role assign` | object: mutation result / dry-run shape (`{ dry_run, mutating, would_send }` when not `--apply`) | json:ApiValidationFailed / json:RouteNotFoundException / json:AccessControlException (Alteryx One `/v4` gateway shape) | Corrected to the live spec contract; sends `{"items":[subjectId]}`. Mutating — not probed. |
...
| Method | Path | Live status | Verified (UTC) | ayx command(s) | Response shape | Error-body flavor | Notes |
|---|---|---|---|---|---|---|---|
| GET | `/v4/open-api-spec` | live 200 | 2026-08-18T18:22Z | `one api coverage`<br>`one api open-api-spec` | object: raw API resource body, JSON-passthrough | json:ApiValidationFailed / json:RouteNotFoundException / json:AccessControlException (Alteryx One `/v4` gateway shape) | Current coverage: `coverage_pct: 61.3`, `inventory_operations: 160`, `inventory_total: 169`, `spec_operations: 235`, `missing: 91`, `stale: 16`, and 7 `outside_spec_namespace` rows. `stale[].commands` is an array. |
...
| Method | Path | Live status | Verified (UTC) | ayx command(s) | Response shape | Error-body flavor | Notes |
|---|---|---|---|---|---|---|---|
| GET | `/v4/workflows` | live 200 | 2026-08-14T16:10Z | `one workflows list` | paginated list: `{ items[], next_page_token, pages_fetched, page_envelopes[] }` (CLI-normalized) | json:ApiValidationFailed / json:RouteNotFoundException / json:AccessControlException (Alteryx One `/v4` gateway shape) | Phase 2 returned 25 items from a real total of 101. Raw DELETE probes with fake ULID `01AAAAAAAAAAAAAAAAAAAAAAAA`: `/svc-workflow/api/v1/assets/...` returned HTTP 404 Express HTML (route-level, no route); `/svc-workflow/api/v2/workflows/...` returned HTTP 404 JSON `NotFoundError` (application-level, route most likely exists but is unwired in the CLI). |
...
| GET | `/svc-workflow/api/v1/tools` | live 200 | 2026-08-14T16:10Z | `one workflows tools` | object: raw API resource body, JSON-passthrough | json:NotFoundError (`{ kind: "NotFoundError", message, errors: [], params: {} }`) for a valid route / unknown resource id | Phase 2 returned 200. Phase 4 found that default text mode renders this response as a long, unformatted JSON blob; use JSON output with `jq` for a readable demo. |
...
| DELETE | `/svc-workflow/api/v2/workflows/{id}` | live 200 | 2026-08-17 (end-to-end: created a resource, deleted it, confirmed removal) | `one workflows delete` | empty object `{}` on success (HTTP 200) / dry-run shape (`{ dry_run, mutating, would_send: null }` when not `--apply`) | json:NotFoundError (`{ kind: "NotFoundError", message, errors: [], params: {} }`) for a valid route / unknown resource id | **Live-verified end-to-end 2026-08-17**, superseding the earlier fake-ULID route-existence inference. Method: duplicated `01M00M9CRWSANK79MBCA0V9VXX` via `POST .../duplicate` (HTTP 201, new id `01M086Y6DG72ERJ2S42JGBB3YY`), then deleted the copy — HTTP 200 with an empty `{}` body. Removal confirmed three ways: the id vanished from `workflows list --all`, the workspace count returned from 103 to its 102 baseline, and `workflows detail` on the id returned `not_found`. Also verified live: an unknown id is rejected with `not_found` **before** any mutating request is sent (the resolve-and-name guard). `one workflows delete` names the target in the confirmation, then gates the live call behind `--apply` plus a TTY confirmation unless `--yes` is also passed (mirrors `one flows delete`) — `--apply --yes` in a non-TTY context sends the DELETE with no interaction, by design, same as every other mutating command here. Irreversible: no restore/trash endpoint exists in this inventory. |
...
| Method | Path | Live status | Verified (UTC) | ayx command(s) | Response shape | Error-body flavor | Notes |
|---|---|---|---|---|---|---|---|
| POST | `/v4/schedules` | live 201 | 2026-08-18T20:24Z | `one scheduling create` | object / dry-run envelope | json:ApiValidationFailed / json:RouteNotFoundException / json:AccessControlException | Created and later deleted disposable schedule `39631`; live gateway accepted a daily trigger and rejected the published `oneTime` variant. |
...
| GET | `/v4/schedules/{id}` | live 200 | 2026-08-18T17:55Z | `one scheduling detail` | object: raw API resource body, JSON-passthrough | json:ApiValidationFailed / json:RouteNotFoundException / json:AccessControlException (Alteryx One `/v4` gateway shape) | Real schedule `39506` returned 200 after the `/v4` repoint. |
| PUT | `/v4/schedules/{id}` | live 200 | 2026-08-18T20:24Z | `one scheduling update` | object / dry-run envelope | json:ApiValidationFailed / json:RouteNotFoundException / json:AccessControlException | Renamed disposable schedule `39631`; uses the spec-documented PUT route. |
...
| POST | `/v4/schedules/{id}/disable` | live 200 | 2026-08-18T20:24Z | `one scheduling disable` | object / dry-run envelope | json:ApiValidationFailed / json:RouteNotFoundException / json:AccessControlException | Disabled disposable schedule `39631`; detail confirmed `enabled: false`. |
| DELETE | `/v4/schedules/{id}` | live 204 | 2026-08-18T20:24Z | `one scheduling delete` | object / dry-run envelope | json:ApiValidationFailed / json:RouteNotFoundException / json:AccessControlException | Deleted disposable schedule `39631`; list returned to the original one-schedule baseline. Per-ID detail remained eventually consistent and served the deleted record. |
...
| Method | Path | Live status | Verified (UTC) | ayx command(s) | Response shape | Error-body flavor | Notes |
|---|---|---|---|---|---|---|---|
| GET | `/v4/connections` | live 200 | 2026-08-14T16:09Z | `one connections list` | paginated list: `{ items[], next_page_token, pages_fetched, page_envelopes[] }` (CLI-normalized) | json:ApiValidationFailed / json:RouteNotFoundException / json:AccessControlException (Alteryx One `/v4` gateway shape) | Phase 2 returned 11 items. |
...
| POST | `/v4/connections/dryRun` | live 400 validation | 2026-08-18T17:55Z | `one connections dry-run` | object: mutation result / dry-run shape (`{ dry_run, mutating, would_send }` when not `--apply`) | json:ApiValidationFailed | A deliberately invalid fixture reached the route and returned 400 `ApiValidationFailed`; this confirms routing and body validation, but not a valid dry-run response shape. |
| GET | `/v4/connections/{id}` | live 200 | 2026-08-14T16:09Z | `one connections detail` | object: raw API resource body, JSON-passthrough | json:ApiValidationFailed / json:RouteNotFoundException / json:AccessControlException (Alteryx One `/v4` gateway shape) | Phase 2 `one connections detail 44865` returned 200. |
...
| GET | `/v4/connections/{id}/permissions/sharedSubjects` | live 200 | 2026-08-14T16:15Z | `one connections permissions`<br>`one connections permissions detail` | paginated list: `{ items[], next_page_token, pages_fetched, page_envelopes[] }` (CLI-normalized) | json:ApiValidationFailed / json:RouteNotFoundException / json:AccessControlException (Alteryx One `/v4` gateway shape) | Phase 3 `one connections permissions list 44865` returned 200, not `RouteNotFoundException`; confirms the repaired `/permissions/sharedSubjects` route. |
| POST | `/v4/connections/share` | live 201 | 2026-08-18T20:26Z | `one connections permissions create` | object / dry-run envelope | json:ApiValidationFailed / json:RouteNotFoundException / json:AccessControlException | Shared connection `44865` with people `4477` and `203464`, verified, then revoked. Empty subject buckets are omitted because the live API rejects them. |
...
| GET | `/v4/jobGroups/{id}` | live 200 | 2026-08-14T16:11Z | `one job-groups detail` | object: raw API resource body, JSON-passthrough | json:ApiValidationFailed / json:RouteNotFoundException / json:AccessControlException (Alteryx One `/v4` gateway shape) | Phase 2 `one job-groups detail 4087561` returned 200. |
...
| Method | Path | Live status | Verified (UTC) | ayx command(s) | Response shape | Error-body flavor | Notes |
|---|---|---|---|---|---|---|---|
| GET | `/v4/apiAccessTokens` | live 200 | 2026-08-14T16:07Z | `one auth diagnose`<br>`one auth status`<br>`one doctor auth`<br>`one token` | paginated list: `{ items[], next_page_token, pages_fetched, page_envelopes[] }` (CLI-normalized) | json:ApiValidationFailed / json:RouteNotFoundException / json:AccessControlException (Alteryx One `/v4` gateway shape) | Phase 2 `one token` returned 200. The tenant has no token detail fixture; the command's result is still a real response, and `doctor auth` independently used this route during discovery. |
...
| GET | `/v4/apiAccessTokens/{tokenId}` | live 404 not_found | 2026-08-18T17:55Z | `one token detail` | object: raw API resource body, JSON-passthrough | json:DataNotFoundException | Fake token id `999999` reached the authenticated route and returned application-level 404 `not_found` (`DataNotFoundException`), not a route-not-found response. |
...
| POST | `/v4/workspaces/{id}/groups` | live 200 | 2026-08-18T20:23Z | `one workspace create-group` | object / dry-run envelope | json:ApiValidationFailed / json:RouteNotFoundException / json:AccessControlException | Created disposable group `01M0B8NVY8ZC9G4P16QSXT9M19`; body requires `name` and `members`. |
...
| PUT | `/v4/workspaces/{id}/groups/{groupId}/roles` | unverified | not probed this session | `one workspace set-group-roles` | object: mutation result / dry-run shape (`{ dry_run, mutating, would_send }` when not `--apply`) | json:ApiValidationFailed / json:RouteNotFoundException / json:AccessControlException (Alteryx One `/v4` gateway shape) | Mutating — JSON body requires `roleIds`. |
| POST | `/v4/workspaces/{id}/groups/{groupId}/users` | live 201 | 2026-08-18T20:23Z | `one workspace add-group-users` | object / dry-run envelope | json:ApiValidationFailed / json:RouteNotFoundException / json:AccessControlException | Added Javier `4477` and Suresh `203464` using repeated `userIds` query parameters. |
...
- **The official spec is incomplete.** `GET /v4/open-api-spec` returned a populated live spec
  with 235 operations in this session — but `GET /v4/workflows` (the cloud-native workflow
...
- **The coverage matcher accepts both path-template styles.** The live spec uses colon parameters
  ...
  comparing them. The current result is 61.3% coverage, with 16 stale and 91 missing operations.
- **Live evidence in this doc comes from one tenant**: the authenticated `local-dev` workspace at
  `https://us1.alteryxcloud.com`, probed 2026-08-18 around 17:55 UTC. The old `/plans/v1/*`,
