...
| GET | `/svc-workflow/api/v1/workflows/{id}/versions/{version}` | documented; current OAuth profile returned 401 | 2026-09-03T23:59Z | `one workflows download` | JSON file written after JSON validation; command envelope reports path and byte count | html:Unauthorized with the current OAuth profile | Alteryx's SDLC Asset Promotion Guide documents this as the workflow JSON download route and uses an API token. The current profile can list assets but lacks authorization for the version-download route; re-probe with an authorized API token. |
