...
_Generated from_ `cargo run -q -p ayx-rs -- --output json catalog list --format full` _on 2026-05-30 00:33:19 UTC._
...
| Name | Path | Safety | Mutating | Summary |
| --- | --- | --- | --- | --- |
| one auto-insights status | `one/auto-insights/status` | read-only | no | Summarize the Alteryx One Auto Insights posture. |
...
| one platform token | `one/platform/token` | read-only | no | List One API access tokens. |
...
| one webhooks test | `one/webhooks/test` | read-only | no | Test webhook settings from JSON payload. |
